 ...
 */
public class ArabicPrediction2 {
	...
	public String[] arabicPrediction(File featuresFile, boolean checkEval) throws JsonParseException, JsonMappingException, IOException {
		...
		for (int i = 0; i < features.segments.size(); i++) {
			...
			System.out.println(i + " - " + segment.name + " - " + segment.getNormalizedBodyChainAsString() + 
					" dotCount=" + segment.dotCount + " dotPos=" + segment.dotPos + " -> " +
					predictionInput.getRows(i));

			// then the first output fires for false, and the second is 0 (see
			// class
			// comment)
		...
		System.out.println("Testing input: " + predictionInput);

		MultiLayerNetwork net = ModelSerializer
				.restoreMultiLayerNetwork(new File("D:\\filetraining\\zhangsuen\\nnmodel.zip"));
		...
		for (int i = 0; i < maxIdxPerRow.length(); i++) {
			...
			String expectedResult = LABELS.get(curIdx);
			ArabicPrediction arabicprediction = new ArabicPrediction();
			
			String[] prediction = arabicprediction
					.arabicPrediction(new File(featuresFile, "zhangsuen\\zhangsuen.Features.json"), false);
			...
			int correctCount = 0;
			for (int i1 = 0; i1 < expectedResult.length(); i1++) {
				char expected = expectedResult.charAt(i1);
				String predicted = i1 < prediction.length ? prediction[i1] : "";
				if (String(expected).equals(predicted)) {
					System.out.println(i1 + " - " + expected + " -> " + predicted + " (OK)");
					correctCount++;
				} else {
					System.out.println(i1 + " - " + expected + " -> " + predicted + " (WRONG)");
				}
			}
		}
	...
	private Object String(char expected) {
	...
	static class Segment {
		...
		public String getNormalizedBodyChainAsString() {
			String result = "";
			for (int bc : normalizedBodyChain) {
				result += bc;
			}
			return result;
		}
	...
	public static void main(String[] args) throws JsonParseException, JsonMappingException, IOException {
		...
		File featuresFile = new File("D:\\filetestingsegmentation\\zhangsuen\\campur.Features.json");
		ArabicPrediction arabicprediction = new ArabicPrediction();
