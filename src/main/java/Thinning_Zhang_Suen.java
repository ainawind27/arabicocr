...
//        threshold = countHwidth != 0 ? (sumHWidth / countHwidth) / 2 : 0;
        ...
        do {
            ...
            for (int y = 1; y + 1 < height; y++) {
                for (int x = 1; x + 1 < width; x++) {
                    ...
                    b = getNeighbor3(binaryImage, y, x);
                    if (binaryImage[y][x] == 1 && b == 1 && a == 1
    ...
    private static int getNeighbor3(int[][] binaryImage, int y, int x) {
    	int [] tmp = new int[8];
    	tmp[2] = binaryImage[y - 1][x];
    	tmp[3] = binaryImage[y - 1][x + 1];
    	tmp[4] = binaryImage[y][x + 1] ;
    	tmp[5] = binaryImage[y + 1][x + 1] ;
    	tmp[6] = binaryImage[y + 1][x] ;
    	tmp[7] = binaryImage[y + 1][x - 1] ;
    	tmp[0] = binaryImage[y][x - 1] ;
    	tmp[1] = binaryImage[y - 1][x - 1] ;
